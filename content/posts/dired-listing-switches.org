...
The trick is, and I would have realised this if I knew how to look up the documentation of Emacs functions, that these are flags that are passed to ~ls~. It makes sense that Emacs would delegate to another tool, it's part of the UNIX philosophy[fn:unix-philosophy] to trust tasks to other tools rather than do them oneself.
